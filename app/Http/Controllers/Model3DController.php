...
class Model3DController extends Controller
{
    ...
    {
        // Проверяем, что пользователь является владельцем модели
 ...
 public function destroy(Model3D $model)
    {
        // Проверяем, что пользователь является владельцем модели ИЛИ админом
        if ($model->user_id !== Auth::id() && !auth()->user()->is_admin) {
            abort(403, 'У вас нет прав для удаления этой модели.');
        }
...
// Добавь этот метод в конец Model3DController
...
// for search and administration users
// Добавь этот метод в Model3DController
